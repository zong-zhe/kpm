...
func TestLoadKclPkgFromTar(t *testing.T) {
	...
	assert.Equal(t, kclPkg.ModFile.Deps.GetOrDefault("oci_konfig", TestPkgDependency).PkgSpec.Version, "0.0.1")
...
func TestLoadPkgFromLock(t *testing.T) {
	...
	assert.Equal(t, kpkg.Dependencies.Deps.GetOrDefault("helloworld", TestPkgDependency).Version, "0.1.2")
	assert.Equal(t, kpkg.Dependencies.Deps.GetOrDefault("helloworld", TestPkgDependency).FullName, "helloworld_")
	...
	assert.Equal(t, kpkg.Dependencies.Deps.GetOrDefault("helloworld", TestPkgDependency).Source.Oci.Tag, "")
}
